...
namespace Nuke.Azure.KeyVault
{
    ...
    public class KeyVaultSecretAttribute : InjectionAttributeBase
    {
        ...
        protected KeyVaultSettings GetSettings ()
        {
            ...
            {
                ControlFlow.Assert(SettingFieldName != null, "There is more then one KeyVaultSettings field defined. Please specify which one to use by setting 'SettingFieldName'");
                attribute = attributes.FirstOrDefault(x => x.Field.Name == SettingFieldName).NotNull($"A KeyVaultSetting field with the name {SettingFieldName} does not exist.").Attribute;
            }
            else
            {
