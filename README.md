# <a href="https://nuke.build"><img src="https://github.com/nuke-build/all/raw/master/images/icon-glossy.png" width="20" /></a> Nuke.Azure.KeyVault
...
class Build : NukeBuild
{
    ...
    [KeyVaultKey] readonly KeyVaultKey KeyVaultKey;
```
...
There are multiple ways to configure the `KeyVaultSettings`, You can use fields with a `[ParameterAttribute]` and pass the names of them, like in the example above.
But you can also pass the names of the environment variable or commandline argument directly:
```c#
[KeyVaultSettings("KEYVAULT_BASE_URL", "KEYVAULT_CLIENT_ID" "KEYVAULT_CLIENT_SECRET")];
```

It's also possible to pass `ClientId` and `BaseUrl` directly as strings, for security reasons this is not possible for the secret.
```c#
[KeyVaultSettings(BaseUrl = "127.0.0.1", ClientId = "clientId", ClientSecretParameterName = "AZ_SECRET_TOKEN")]
```
...
```c#
    ...
    //[KeyVaultSecret] string SecretFromOtherSettings; Not working! If multiple settings are defined 'SettingFieldName' must be defined.
